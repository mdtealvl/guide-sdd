...
| test_edit_ban | `test_edit_ban.ps1` / `.sh` | generic | the Engineer touched no test file (QA⊥Engineer, structurally) | 6 |
...
| suite_green | `suiteCmd` wrapper | generic | the project suite exits 0 | 6, 7 |
...
| qa_import_ban | `qa_import_ban.template.ps1` / `.sh` | **project** | QA tests don't import production internals (structural half of QA⊥impl — otherwise honor-system) | 7 |
...
- **Per-stage:** `link_check` + `prose_check` after any spec edit (Stage 3); `coverage_check --plan` at Stage 4;
  `coverage_check` at end of Stage 5; `test_edit_ban` + `suite_green` at end of Stage 6.
  ...
  | test_edit_ban | `pwsh gates/test_edit_ban.ps1 <baseRef>` | `sh gates/test_edit_ban.sh <baseRef>` |
...
| `paths.tests` / `testGlobs` | globs identifying test files (coverage_check scans tags; test_edit_ban forbids edits). |
| `baseRef` | the merge base for diffs (`main`). |
