...
## v1.14.0 — Concurrent personas, nested-repo gate banks (per SDD-amend-v1.14.0, 2026-09-22)

No invariant change; still ten. Persona guard: `agent_type` sets the persona with no marker (concurrent
QA/Engineer, PG.1), a `qa` Bash path-token tripwire (PG.4), an attributed `engineer` sweep scoped to its
...
Persona guard: qa may read files matching testGlobs, and directories under a `<dir>/**` testGlob, inside paths.code; `..` segments under paths.code denied (PG.3b); closes #4.
